...
static const uint NumSamples = 64 * 16;
...
void main(uint3 ThreadID : SV_DispatchThreadID)
{
	...
	for(uint i=0; i<NumSamples; ++i) {
		...
		irradiance += 2.0 * sRGB2Lin(inputTexture.SampleLevel(linear_wrap_sampler, Li, 0).rgb) * cosTheta;
	}
	irradiance /= float(NumSamples);

	outputTexture[ThreadID] = float4(Lin2sRGB(irradiance), 1.0);
}
